...
def register():
    ...
    hashed_password = hash_password(new_pass)  # Хешируем пароль
...
def _auth():
    ...
    if choice == "1":
        ...
        hashed_password = hash_password(pass_user)
        
        cursor.execute("SELECT * FROM users WHERE username = ? AND password = ?", (auth_user, hashed_password))
        if cursor.fetchone():
